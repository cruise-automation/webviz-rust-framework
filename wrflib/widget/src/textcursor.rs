...
impl TextCursorSet {
    ...
    fn fuse_adjacent(&mut self, text_buffer: &TextBuffer) {
        ...
        loop {
            ...
            if my_end >= next_start {
                ...
                if my_end < next_end {
                    // otherwise just remove the next
        ...
        for cursor in &mut self.set {
            ...
            if start == end && start > 0 && start < text_buffer.flat_text.len() {
                ...
                if pch == '{' && nch == '}' || pch == '(' && nch == ')' || pch == '[' && nch == ']' {
                    ...
                    // just do a newline with indenting
                    let mut text = String::new();
                    text.push('\n');
                    for _ in 0..pre_spaces {
                        text.push(' ');
    ...
    pub fn insert_around(&mut self, pre: &str, post: &str, text_buffer: &mut TextBuffer) {
        ...
        for cursor in &mut self.set {
            ...
            if start != end {
                ...
                // only insert post if next char is whitespace newline, : , or .
            ...
            if start == end {
                let op = if let Some((rstart, l1ws, l1len, l2ws)) = text_buffer.calc_deletion_whitespace(start) {
                    if l1ws == l1len {
                        ...
                        // if start == rstart + l1len we are at the end of a line
    ...
    /// Clamp the [`TextCursor`] positons to the total text size of the provided [`TextBuffer`] size.
    ///
    /// This is useful in making sure that the position of the [`TextCursor`] is correct when
    /// you set_value of the [`TextBuffer`] to be smaller than before.
    pub fn clamp_to_text_buffer(&mut self, text_buffer: &TextBuffer) {
        // calc_char_count() is an expensive operation, if editor becomes unperformant,
        // then we might want to create a system of caching the result of this function call.
    ...
    pub fn get_nearest_token_chunk_boundary(left: bool, offset: usize, text_buffer: &TextBuffer) -> usize {
        ...
        for i in 0..token_chunks.len() {
            ...
            if offset >= token_chunks[i].offset && offset < token_chunks[i].offset + token_chunks[i].len {
                if left {
                    // we want to to the beginning of the prev token
            ...
            if offset >= token_chunks[i].offset && offset < token_chunks[i].offset + token_chunks[i].len {
                ...
                if token_chunks[i].token_type == TokenType::String || token_chunks[i].token_type == TokenType::CommentChunk {
                    if token_chunks[i].len <= 2 {
                        ...
                        // scan for the nearest left and right space in the string
...
impl DrawCursors {
    ...
    pub fn process_newline(&mut self) {
        if !self.first {
            // we have some selection data to emit
