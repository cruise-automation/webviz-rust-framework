...
impl Splitter {
    ...
    pub fn handle(&mut self, cx: &mut Cx, event: &mut Event) -> SplitterEvent {
        ...
        match event.hits(cx, &self.component_base, HitOpt { margin: self._hit_state_margin, ..Default::default() }) {
            Event::FingerDown(fe) => {
    ...
    pub fn set_splitter_state(&mut self, align: SplitterAlign, pos: f32, axis: Axis) {
        ...
        self.pos = pos;
        match self.axis {
            Axis::Horizontal => self._hit_state_margin = Some(Margin { l: 0., t: 3., r: 0., b: 7. }),
            Axis::Vertical => self._hit_state_margin = Some(Margin { l: 3., t: 0., r: 7., b: 0. }),
        }
    }
    ...
    pub fn end_draw(&mut self, cx: &mut Cx) {
        ...
        self.component_base.register_component_area(cx, self.bg.area())
    }
}
