...
pub fn write_block_to_tcp_stream(tcp_stream: &mut TcpStream, msg_buf: &[u8], digest: Digest) -> HubResult<()> {
    ...
    let mut dwd_write = DigestWithData { digest, data: compressed.len() as u64 };
...
impl HubClient {
    pub fn connect_to_server(digest: Digest, server_address: SocketAddr, hub_log: HubLog) -> HubResult<HubClient> {
        ...
        let read_thread = {
            ...
            let server_hubaddr = server_hubaddr;
                ...
                loop {
                    match read_block_from_tcp_stream(&mut tcp_stream, digest.clone()) {
                        ...
                        Err(e) => {
                            ...
                            tx_read
                                .send(FromHubMsg { from: server_hubaddr, msg: HubMsg::ConnectionError(e.clone()) })
                                .expect("tx_read.send fails - should never happen");
                            // lets break rx write
                            let _ = tx_write_copy.send(ToHubMsg { to: HubMsgTo::Hub, msg: HubMsg::ConnectionError(e) });
                            return;
                        }
                    }
                }
        ...
        let write_thread = {
            ...
            let hub_log = hub_log;
            std::thread::spawn(move || {
                // this one cannot send to the read channel.
                while let Ok(cth_msg) = rx_write.recv() {
                    ...
                    if let Err(e) = write_block_to_tcp_stream(&mut tcp_stream, &msg_buf, digest.clone()) {
                        ...
                        let _ = tcp_stream.shutdown(Shutdown::Both);
                        let _ = tx_read.send(FromHubMsg { from: server_hubaddr, msg: HubMsg::ConnectionError(e) });
                        return;
                    }
                }
            })
    ...
    pub fn alloc_uid(&mut self) -> HubUid {
        self.uid_alloc += 1;
        HubUid { addr: self.own_addr, id: self.uid_alloc }
    }
