...
const INCLUDED_FILES: &[&str] = &[".json", ".toml", ".js", ".rs", ".txt", ".text", ".ron", ".html"];
const EXCLUDED_FILES: &[&str] = &["key.ron"];
const EXCLUDED_DIRS: &[&str] = &["target", ".git", ".github", "edit_repo"];
...
impl HubBuilder {
        ...
        F: Fn(&mut HubBuilder, FromHubMsg) -> Result<(), HubWsError> + Clone + Send + 'static,
    {
        ...
        let _thread = {
            ...
            std::thread::spawn(move || {
                route_send.send(ToHubMsg { to: HubMsgTo::All, msg: HubMsg::ConnectBuilder(builder.to_string()) });
                        ...
                        std::thread::spawn(move || {
                            ...
                            if let Some(is_build_uid) = is_build_uid {
                                if result.is_ok() {
                                    hub_builder
                                        .route_send
                                        .send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::BuildFailure { uid: is_build_uid } });
                                } else {
                                    hub_builder
                                        .route_send
                                        .send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::BuildSuccess { uid: is_build_uid } });
                                }
                            }
                        })
                    };
        ...
        F: Fn(&mut HubBuilder, FromHubMsg) -> Result<(), HubWsError> + Clone + Send + 'static,
    {
        ...
        let (message, path, mount) = match args[1].as_ref() {
            ...
            "list" => {
                ...
                (HubMsg::ListPackagesRequest { uid: HubUid::zero() }, args[2].clone(), None)
            }
            "build" => {
                if args.len() != 5 {
                    return print_help();
            ...
            "index" => {
                ...
                (
                    HubMsg::BuilderFileTreeRequest { uid: HubUid::zero(), create_digest: false },
                    args[2].clone(),
                    Some(args[3].clone()),
                )
            }
        ...
        if let Ok(mut workspaces) = workspaces.lock() {
            workspaces.insert("main".to_string(), rel_to_abs_path(&abs_cwd_path, &path));
        };
                    ...
                    HubMsg::BuilderFileTreeResponse { tree, .. } => {
                        ...
                        if let BuilderFileTreeNode::Folder { folder, .. } = tree {
                            let ron = BuilderFileTreeNode::Folder {
                                name: "".into(),
                                digest: None,
                                folder: vec![BuilderFileTreeNode::Folder {
                                    name: "main".into(),
                                    digest: None,
                                    folder: vec![BuilderFileTreeNode::Folder {
                                        name: mount.unwrap(),
        ...
        let result = event_handler(&mut hub_builder, FromHubMsg { from: HubAddr::None, msg: message });
        if result.is_ok() {
            let _ = tx_write.send((
                HubAddr::None,
                ToHubMsg { to: HubMsgTo::All, msg: HubMsg::BuildSuccess { uid: HubUid { id: 0, addr: HubAddr::None } } },
            ));
        } else {
            let _ = tx_write.send((
                HubAddr::None,
                ToHubMsg { to: HubMsgTo::All, msg: HubMsg::BuildFailure { uid: HubUid { id: 0, addr: HubAddr::None } } },
            ));
        }
        let _ = thread.join();
    }
    ...
    pub fn default(&mut self, htc: FromHubMsg) -> Result<(), HubWsError> {
        let ws = self;
        match htc.msg {
            HubMsg::BuilderConfig { uid, config } => ws.set_config(uid, config),
            HubMsg::BuilderFileTreeRequest { uid, create_digest } => {
                let tree = ws.workspace_file_tree(create_digest, INCLUDED_FILES, EXCLUDED_FILES, EXCLUDED_DIRS);
                ws.route_send
                    .send(ToHubMsg { to: HubMsgTo::Client(htc.from), msg: HubMsg::BuilderFileTreeResponse { uid, tree } });
                Ok(())
            }
    ...
    pub fn workspace_split_from_path(&mut self, uid: HubUid, path: &str) -> Result<(String, String, String), HubWsError> {
        if let Some(workspace_pos) = path.find('/') {
            ...
            return Ok((abs_dir, workspace.to_string(), rest.to_string()));
        }
        Err(self.error(uid, format!("Builder {} path {} incorrent", self.builder, path)))
    }
    ...
    pub fn get_workspace_abs(&mut self, uid: HubUid, workspace: &str) -> Result<String, HubWsError> {
        ...
        Err(self.error(uid, format!("Builder {} workspace {} not found", self.builder, workspace)))
    }
    ...
    pub fn program_run(&mut self, uid: HubUid, path: &str, args: &[&str]) -> Result<(), HubWsError> {
        ...
        if let Ok(mut processes) = self.processes.lock() {
            processes.push(HubProcess { uid, process });
        };

        let builder = self.builder.clone();
        route_mode.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::ProgramBegin { uid } });
        ...
        fn try_parse_stderr(uid: HubUid, builder: &str, workspace: &str, stderr: &Vec<String>, route_send: &HubRouteSend) {
            ...
            for line in stderr {
                ...
                if !tracing_panic && (trimmed.starts_with("thread '") || trimmed.starts_with("0:")) {
                    // this is how we recognise a stacktrace start..Very sturdy.
                    tracing_panic = true;
                ...
                if tracing_panic {
                    ...
                    route_send.send(ToHubMsg {
                        to: HubMsgTo::UI,
                        msg: HubMsg::LogItem { uid, item: HubLogItem::Error(trimmed.clone()) },
                    });
                }
            }
            ...
            for panic_line in &panic_stack {
                    ...
                    && !panic_line.starts_with("at src/libpanic_unwind/")
                {
                    if let Some(end) = panic_line.find(':') {
                        ...
                        let proc_line =
                            if let Ok(pl) = panic_line.get((end + 1)..(panic_line.len() - 1)).unwrap().parse::<usize>() {
                                pl
                            } else {
                                0
                            };
                    ...
                    if let Some((is_stderr, line)) = line {
                        if is_stderr {
                            ...
                            route_mode.send(ToHubMsg {
                                to: HubMsgTo::UI,
                                msg: HubMsg::LogItem { uid, item: HubLogItem::Message(line.clone()) },
                            });
                        }
                    }
                }
        ...
        route_mode.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::ProgramEnd { uid } });
    ...
    pub fn cannot_find_build(&mut self, uid: HubUid, package: &str, target: &str) -> Result<(), HubWsError> {
        Err(self.error(uid, format!("Builder {} Cannot find package {} and target {}", self.builder, package, target)))
    }
    ...
    pub fn cargo(
        ...
        route_send.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::CargoBegin { uid } });
        ...
        while let Ok(line) = rx_line.recv() {
            if let Some((is_stderr, line)) = line {
                ...
                {
                    route_send
                        .send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::LogItem { uid, item: HubLogItem::Error(line.clone()) } });
                }
                    ...
                    Ok(parsed) => {
                        if let Some(message) = &mut parsed.message {
                            ...
                            for i in 0..spans.len() {
                                ...
                                route_send.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::LogItem { uid, item } });
                            }
                        } else {
                            route_send.send(ToHubMsg {
                                to: HubMsgTo::UI,
                            ...
                            if errors.is_empty() {
                                ...
                                if let Some(executable) = &parsed.executable {
                                    if !executable.ends_with(".rmeta") && abs_root_path.len() + 1 < executable.len() {
                                        ...
                                        build_result = BuildResult::Executable {
                                            path: format!("{}/{}", workspace, last).replace("\\", "/"),
                                        };
                                    }
                                }
                                ...
                                if let Some(filenames) = &parsed.filenames {
                                    for filename in filenames {
                                        if filename.ends_with(".wasm") && abs_root_path.len() + 1 < filename.len() {
                                            ...
                                            build_result = BuildResult::Wasm { path };
                                        }
                                    }
                                }
                            } else {
        ...
        route_send.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::CargoEnd { uid, build_result: build_result.clone() } });
    ...
    pub fn packages_response(&mut self, from: HubAddr, uid: HubUid, packages: Vec<HubPackage>) {
        self.route_send.send(ToHubMsg { to: HubMsgTo::Client(from), msg: HubMsg::ListPackagesResponse { uid, packages } });
    }

    pub fn error(&mut self, uid: HubUid, msg: String) -> HubWsError {
        self.route_send.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::LogItem { uid, item: HubLogItem::Error(msg.clone()) } });

        HubWsError::Error(msg)
    }

    pub fn message(&mut self, uid: HubUid, msg: String) {
        self.route_send.send(ToHubMsg { to: HubMsgTo::UI, msg: HubMsg::LogItem { uid, item: HubLogItem::Message(msg) } });
    }
    ...
    pub fn file_write(&mut self, from: HubAddr, uid: HubUid, path: &str, data: Vec<u8>) {
        if let Ok((abs_dir, _project, sub_path)) = self.workspace_split_from_path(uid, path) {
            if path.contains("..") {
                ...
                println!("file_write got relative path, ignoring {}", path);
                return;
            }
            ...
            self.route_send.send(ToHubMsg {
                ...
                msg: HubMsg::FileWriteResponse { uid, path: path.to_string(), done },
            });
        }
    }
                    ...
                    if let Ok(entry) = entry {
                        if let Ok(ty) = entry.file_type() {
                            if let Ok(name) = entry.file_name().into_string() {
                                if ty.is_dir() {
                                    if dir_ex.iter().any(|dir| *dir == name) {
                                        continue;
                                    }
                                    let folder =
                                        read_recur(&format!("{}/{}", path, name), create_digest, ext_inc, file_ex, dir_ex);
                                    ...
                                    if file_ex.iter().any(|file| *file == name) {
                                        continue;
                                    }
                                    if ext_inc.iter().any(|ext| name.ends_with(ext)) {
                                        if create_digest {}
                                        ret.push(BuilderFileTreeNode::File { digest: None, name });
                                    }
                                }
                            }
                        }
...
/*
{
    ...
    "package_id": "makepad-internal-iter 0.1.0 (path+file:///Users/admin/wrflib/edit_repo/main/vector/internal_iter)",
    "target": {
        "kind": ["lib"],
        "crate_types": ["lib"],
        "name": "makepad-internal-iter",
        "src_path": "/Users/admin/wrflib/edit_repo/main/vector/internal_iter/src/lib.rs",
        "edition": "2018",
        "doctest": true,
        "test": true
    },
