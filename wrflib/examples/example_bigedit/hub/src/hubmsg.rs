...
pub enum HubMsg {
    ...
    BuilderConfig { uid: HubUid, config: HubBuilderConfig },

    // make client stuff
    Build { uid: HubUid, workspace: String, package: String, config: String },

    BuildFailure { uid: HubUid },

    BuildSuccess { uid: HubUid },

    BuildKill { uid: HubUid },

    CargoBegin { uid: HubUid },

    LogItem { uid: HubUid, item: HubLogItem },

    CargoArtifact { uid: HubUid, package_id: String, fresh: bool },

    CargoEnd { uid: HubUid, build_result: BuildResult },

    ListPackagesRequest { uid: HubUid },

    ListPackagesResponse { uid: HubUid, packages: Vec<HubPackage> },

    ProgramKill { uid: HubUid },

    ProgramRun { uid: HubUid, path: String, args: Vec<String> },

    ProgramBegin { uid: HubUid },

    ProgramEnd { uid: HubUid },

    BuilderFileTreeRequest { uid: HubUid, create_digest: bool },

    BuilderFileTreeResponse { uid: HubUid, tree: BuilderFileTreeNode },

    ListBuildersRequest { uid: HubUid },

    ListBuildersResponse { uid: HubUid, builders: Vec<String> },

    FileReadRequest { uid: HubUid, path: String },

    FileWriteRequest { uid: HubUid, path: String, data: Vec<u8> },

    FileWriteResponse { uid: HubUid, path: String, done: bool },
}

impl HubMsg {
    pub fn is_blocking(&self) -> bool {
        matches!(self, HubMsg::BuilderConfig { .. } | HubMsg::FileWriteRequest { .. })
    }
}
...
impl Ord for BuilderFileTreeNode {
    fn cmp(&self, other: &BuilderFileTreeNode) -> Ordering {
        match self {
            BuilderFileTreeNode::File { name: lhs, .. } => match other {
                BuilderFileTreeNode::File { name: rhs, .. } => lhs.cmp(rhs),
                BuilderFileTreeNode::Folder { name: _rhs, .. } => Ordering::Greater,
            },
            BuilderFileTreeNode::Folder { name: lhs, .. } => match other {
                BuilderFileTreeNode::File { name: _rhs, .. } => Ordering::Less,
                BuilderFileTreeNode::Folder { name: rhs, .. } => lhs.cmp(rhs),
            },
        }
    }
}
...
impl HubError {
    pub fn new(msg: &str) -> HubError {
        HubError { msg: msg.to_string() }
    }
}
