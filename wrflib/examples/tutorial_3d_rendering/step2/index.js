...
const loadSTLIntoGeometry = async (assetUrl) => {
    await wrflib.initialize({ filename: '/target/wasm32-unknown-unknown/debug/tutorial_3d_rendering_step2.wasm' });

    const [vertices, normals] = await wrflib.callRust("parse_stl");
...
const init = async () => {
    ...
    const geometry = await loadSTLIntoGeometry("/wrflib/examples/tutorial_3d_rendering/teapot.stl");
