...
impl FileTree {
    ...
    pub fn draw(&mut self, cx: &mut Cx) {
        ...
        while let Some((depth, index, len, node)) = file_walker.walk() {
            ...
            let node_bg_turtle = self.node_bg.begin_turtle(cx, node_layout, node_draw.animator.get_vec4(0));
            ...
            match node {
                FileNode::Folder { name, state, .. } => {
                    ...
                    let text_turtle = cx.begin_turtle(Layout { padding: Padding::default(), ..self.node_layout });
            ...
            self.node_bg.end_turtle(cx, node_bg_turtle);
        ...
        if self.row_height > 0. {
            ...
            while y < rect_now.size.y {
                let turtle = self.node_bg.begin_turtle(
                    cx,
                    Layout {
                        walk: Walk::wh(Width::Fill, Height::Fix((rect_now.size.y - y).min(self.row_height))),
                        ..Layout::DEFAULT
                    },
                    if counter & 1 == 0 { LIST_ANIMS_COLOR_BG_EVEN } else { LIST_ANIMS_COLOR_BG_ODD },
                );
                self.node_bg.end_turtle(cx, turtle);
                cx.turtle_new_line();
                y += self.row_height;
                counter += 1;
            }
        ...
        if let Some(mv) = &self._drag_move {
            ...
            while let Some((_depth, _index, _len, node)) = file_walker.walk() {
                ...
                if node_draw.marked != 0 {
                    ...
                    let turtle = self.drag_bg.begin_turtle(
                        cx,
                        Layout {
                    ...
                    TextIns::draw_walk(cx, name, &tree_text_props);
                    self.drag_bg.end_turtle(cx, turtle);
                }
            }
