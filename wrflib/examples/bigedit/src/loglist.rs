...
const LAYOUT_ITEM: Layout = Layout { walk: Walk::wh(Width::Fill, Height::Fix(ITEM_HEIGHT)), ..Layout::DEFAULT };
const LAYOUT_ITEM_CONTENT: Layout = Layout { walk: Walk::wh(Width::Compute, Height::Fix(ITEM_HEIGHT)), ..Layout::DEFAULT };
...
impl LogItemDraw {
    fn draw_icon(&mut self, cx: &mut Cx, icon_type: CodeIconType) {
        ...
        cx.end_turtle(icon_turtle);
    }

    fn draw_log_path(&mut self, cx: &mut Cx, path: &str, row: usize) {
        let text_turtle = cx.begin_turtle(LAYOUT_ITEM_CONTENT);
        cx.begin_center_y_align();
    ...
    fn draw_log_item(&mut self, cx: &mut Cx, _index: usize, list_item: &mut ListItem, log_item: &HubLogItem, anim_default: Anim) {
        ...
        let turtle = self.item_bg.begin_turtle(cx, LAYOUT_ITEM, Vec4::default());
        ...
        self.item_bg.end_turtle(cx, turtle);
        list_item.set_area(cx, self.item_bg.area());
    }

    fn draw_status_line(&mut self, cx: &mut Cx, counter: usize, bm: &BuildManager) {
        let turtle = self.item_bg.begin_turtle(
            cx,
            LAYOUT_ITEM,
