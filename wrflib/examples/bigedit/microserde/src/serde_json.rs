    ...
    pub fn next_tok(&mut self, i: &mut Chars) -> Result<(), DeJsonErr> {
        ...
        match self.cur {
            ...
            'a'..='z' | 'A'..='Z' | '_' => {
                ...
                while self.cur >= 'a' && self.cur <= 'z' || self.cur >= 'A' && self.cur <= 'Z' || self.cur == '_' {
...
unsafe fn de_json_array_impl_inner<T>(top: *mut T, count: usize, s: &mut DeJsonState, i: &mut Chars) -> Result<(), DeJsonErr>
where
    T: DeJson,
{
    s.block_open(i)?;
