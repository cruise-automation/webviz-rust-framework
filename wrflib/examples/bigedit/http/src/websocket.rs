...
pub struct WebSocket {
    ...
    is_ping: bool,
    is_pong: bool,
    #[allow(dead_code)] // currently is_partial is never used
    is_partial: bool,
    is_masked: bool,
...
impl WebSocket {
    ...
    fn parse_head(&mut self, input: &[u8]) -> bool {
        while self.head_expected > 0 && self.input_read < input.len() && self.head_written < self.head.len() {
        ...
        loop {
            match self.state {
                ...
                WebSocketState::Len2 => {
                    ...
                    self.data_len = u16::from_be_bytes(self.head[0..2].try_into().unwrap()) as usize;
                    if self.is_masked {
                        self.to_state(WebSocketState::Mask);
                    } else {
                        self.to_state(WebSocketState::Data);
                ...
                WebSocketState::Len8 => {
                    ...
                    self.data_len = u64::from_be_bytes(self.head[0..8].try_into().unwrap()) as usize;
                    if self.is_masked {
                        self.to_state(WebSocketState::Mask);
                    } else {
                        self.to_state(WebSocketState::Data);
