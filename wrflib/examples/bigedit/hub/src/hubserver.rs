...
impl HubServer {
    pub fn start_hub_server(digest: Digest, config: &HubServerConfig, hub_router: &HubRouter) -> Option<HubServer> {
        ...
        let listener = if let Ok(listener) = TcpListener::bind(listen_address) {
            ...
            println!("start_hub_server bind server address");
            return None;
        };
        ...
        let shared = Arc::new(Mutex::new(HubServerShared { connections: Vec::new(), terminate: false }));
                            ...
                            loop {
                                match read_block_from_tcp_stream(&mut tcp_stream, digest.clone()) {
                                    Ok(msg_buf) => {
                                        ...
                                        tx_pump.send((peer_addr, cth_msg)).expect("tx_pump.send fails - should never happen");
                                    }
                                    Err(e) => {
                                        let _ = tcp_stream.shutdown(Shutdown::Both);
                                        let _ = tx_pump
                                            .send((
                                                peer_addr,
                                                ToHubMsg { to: HubMsgTo::Hub, msg: HubMsg::ConnectionError(e.clone()) },
                                            ))
                                            .expect("tx_pump.send fails - should never happen");
                                        // lets break rx write
                                        let _ =
                                            tx_write_copy.send(FromHubMsg { from: peer_addr, msg: HubMsg::ConnectionError(e) });
                                        return;
                                    }
                                }
                            }
                    ...
                    let _write_thread = {
                        ...
                        std::thread::spawn(move || {
                            while let Ok(htc_msg) = rx_write.recv() {
                                match &htc_msg.msg {
                                    HubMsg::ConnectionError(_) => {
                                        // we are closed by the read loop
                                        let _ = tcp_stream.shutdown(Shutdown::Both);
                                        break;
                                    }
                                    _ => (),
                                ...
                                if let Err(e) = write_block_to_tcp_stream(&mut tcp_stream, &msg_buf, digest.clone()) {
                                    ...
                                    let _ = tcp_stream.shutdown(Shutdown::Both);
                                    tx_pump
                                        .send((peer_addr, ToHubMsg { to: HubMsgTo::Hub, msg: HubMsg::ConnectionError(e) }))
                                        .expect("tx_pump.send fails - should never happen");
                                }
                            }
        ...
        let hub_server = HubServer { shared, listen_address: Some(listen_address), listen_thread: Some(listen_thread) };
    ...
    pub fn terminate(&mut self) {
        ...
        if let Some(listen_address) = self.listen_address {
            ...
            if TcpStream::connect(listen_address).is_ok() {
