...
impl HttpServer {
    pub fn start_http_server(
            ...
            std::thread::spawn(move || {
                for tcp_stream in listener.incoming() {
                    ...
                    let _read_thread = std::thread::spawn(move || {
                        ...
                        if header.path.ends_with("/key.ron")
                            || header.path.contains("..")
                            || header.path_no_slash.starts_with('/')
                        {
                            return http_error_out(tcp_stream, 500);
                        }

                        if header.path.starts_with("/$watch") {
                            // its a watcher wait for the finish
                        ...
                        let file_path = if let Some(file_pos) = header.path_no_slash.find('/') {
                            ...
                            if let Ok(workspaces) = workspaces.lock() {
                                workspaces.get(workspace).map(|abs_path| format!("{}/{}", abs_path, rest))
                            } else {
                                None
                            }
                        } else {
                        ...
                        let file_path =
                            if file_path.ends_with('/') { format!("{}/{}", file_path, "index.html") } else { file_path };

                        if let Ok(mut shared) = shared.lock() {
                            if !shared.files_read.iter().any(|v| *v == header.path_no_slash) {
        ...
        Some(HttpServer { listen_thread: Some(listen_thread), listen_address: Some(listen_address), shared })
    }
    ...
    pub fn send_file_change(&mut self, path: &str) {
        if let Ok(shared) = self.shared.lock() {
            if !shared.files_read.iter().any(|v| *v == path) {
                return;
            }
        }
