...
impl HubRouteSend {
    pub fn is_own_addr(&self, addr: &HubAddr) -> bool {
        match self {
            ...
            HubRouteSend::Direct { own_addr, .. } => *own_addr == *addr,
        }
    }
    ...
    pub fn alloc_uid(&mut self) -> HubUid {
        match self {
            HubRouteSend::Networked { own_addr_arc, uid_alloc, .. } => {
                ...
                if let Ok(own_addr) = own_addr_arc.lock() {
                    if let Some(own_addr) = *own_addr {
                        return HubUid { addr: own_addr, id };
                    }
                }
            }
...
impl HubRouter {
    pub fn alloc_local_addr(&mut self) -> HubAddr {
        self.local_uid += 1;
        HubAddr::Local { uid: self.local_uid }
    }
    ...
    pub fn connect_direct(&mut self, route_type: HubRouteType, tx_write: mpsc::Sender<FromHubMsg>) -> HubRouteSend {
        ...
        if let Ok(mut routes) = self.routes.lock() {
            routes.push(HubRoute { route_type, peer_addr: own_addr, tcp_stream: None, tx_write })
        };

        HubRouteSend::Direct { uid_alloc: Arc::new(Mutex::new(0)), tx_pump, own_addr }
    }
    ...
    pub fn start_hub_router(hub_log: HubLog) -> HubRouter {
        ...
        let router_thread = {
            ...
            std::thread::spawn(move || {
                ...
                while let Ok((from, cth_msg)) = rx_pump.recv() {
                    let to = cth_msg.to;
                    let htc_msg = FromHubMsg { from, msg: cth_msg.msg };
                    ...
                    if let Ok(mut routes) = routes.lock() {
                        ...
                        match to {
                            ...
                            HubMsgTo::Client(addr) => {
                                // find our specific addr and send
                            ...
                            HubMsgTo::Builder(to_ws_name) => {
                                for route in routes.iter() {
                                    match &route.route_type {
                                        HubRouteType::Builder(ws_name) => {
                                            if to_ws_name == *ws_name {
                                                route.tx_write.send(htc_msg.clone()).expect("Could not tx_write.send");
                                            }
                                        }
                                        HubRouteType::Clone(ws_name) => {
                                            if to_ws_name == *ws_name {
                                                route.tx_write.send(htc_msg.clone()).expect("Could not tx_write.send");
                                            }
                                        }
                                ...
                                match &htc_msg.msg {
                                    HubMsg::ConnectionError(e) => {
                                        ...
                                        if let Some(pos) = routes.iter().position(|c| c.peer_addr == htc_msg.from) {
                                            hub_log
                                                .log(&format!("Server closing connection {:?} from error {:?}", htc_msg.from, e));
                                    ...
                                    HubMsg::ListBuildersRequest { uid } => {
                                        ...
                                        for route in routes.iter() {
                                            if let HubRouteType::Builder(ws_name) = &route.route_type {
                                                builders.push(ws_name.to_string())
                                            }
                                        }
                                        ...
                                        if let Some(route) = routes.iter().find(|c| c.peer_addr == htc_msg.from) {
                                            route
                                                ...
                                                .send(FromHubMsg {
                                                    from: htc_msg.from,
                                                    msg: HubMsg::ListBuildersResponse { uid: *uid, builders },
                                                })
        ...
        HubRouter { tx_pump, router_thread: Some(router_thread), local_uid: 1, routes }
    }
}
