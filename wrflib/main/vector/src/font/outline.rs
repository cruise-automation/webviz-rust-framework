...
impl Outline {
    ...
    pub(crate) fn contours(&self) -> Contours {
        Contours { contour_start: 0, contour_ends: self.contour_ends.iter().cloned(), points: &self.points }
    }

    /// Returns an slice of the points of `self`.
    pub(crate) fn points(&self) -> &[OutlinePoint] {
        &self.points
    }

    /// Returns an iterator over the path commands that correspond to `self`.
    pub fn commands(&self) -> Commands {
        Commands { contours: self.contours() }
    }

    /// Returns a mutable slice of the points of `self`.
    pub(crate) fn points_mut(&mut self) -> &mut [OutlinePoint] {
        &mut self.points
    }

    /// Returns a builder for a contour.
    pub(crate) fn begin_contour(&mut self) -> ContourBuilder {
        ContourBuilder { contour_ends: &mut self.contour_ends, points: &mut self.points }
    }
}
...
impl<'a> Iterator for Contours<'a> {
    ...
    fn next(&mut self) -> Option<Contour<'a>> {
        self.contour_ends.next().map(|contour_end| {
            ...
            Contour { points: &self.points[contour_start..contour_end] }
        })
    }
}
...
impl<'a> Contour<'a> {
    pub(crate) fn points(&self) -> &'a [OutlinePoint] {
        self.points
    }
}
                ...
                if first_on_curve_point.is_none() {
                    ...
                    match (last_off_curve_point, point.is_on_curve) {
                        ...
                        (Some(last_point), false) => {
                            if !f(PathCommand::QuadraticTo(last_point, last_point.lerp(point.point, 0.5))) {
                                return false;
                            }
                            last_off_curve_point = Some(point.point);
                        }
            ...
            if let Some(first_on_curve_point) = first_on_curve_point {
                match (last_off_curve_point, first_off_curve_point) {
                    ...
                    (None, Some(first_off_curve_point)) => {
                        if !f(PathCommand::QuadraticTo(first_off_curve_point, first_on_curve_point)) {
                            return false;
                        }
                    }
...
#[derive(Debug)]
pub(crate) struct ContourBuilder<'a> {
...
impl<'a> ContourBuilder<'a> {
    ...
    pub(crate) fn push(&mut self, point: OutlinePoint) {
