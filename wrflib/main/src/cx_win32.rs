...
impl Win32App {
    pub(crate) fn new() -> Win32App {
        let class_name_wstr: Vec<u16> = OsStr::new("WrfWindow").encode_wide().chain(Some(0).into_iter()).collect();
    ...
    pub(crate) unsafe extern "system" fn window_class_proc(hwnd: HWND, msg: UINT, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
        ...
        match msg {
            ...
            winuser::WM_DESTROY => {
                ...
                window.do_callback(&mut vec![Event::WindowClosed(WindowClosedEvent { window_id: window.window_id })]);
            }
...
// reworked from winit windows platform
// https://github.com/rust-windowing/winit/blob/eventloop-2.0/src/platform_impl/windows/dpi.rs
...
impl DpiFunctions {
    ...
    /*
    pub(crate) fn get_monitor_dpi(hmonitor: HMONITOR) -> Option<u32> {
        unsafe {
