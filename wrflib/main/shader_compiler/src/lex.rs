...
{
    fn read_token_with_span(&mut self) -> Result<TokenWithSpan, ParseError> {
        ...
        let token = match (self.ch_0, self.ch_1) {
            ...
            ('-', ch) | ('.', ch) | (ch, _) if ch.is_ascii_digit() => {
                ...
                if has_frac_part || has_exp_part {
                    ...
                    Token::Lit(Lit::Int(
                        string.parse::<i32>().map_err(|_| span.error(self, "overflowing integer literal".into()))?,
                    ))
                }
            }
            ('-', _) => {
            ...
            _ => return Err(span.error(self, format!("unexpected character `{}`", self.ch_0))),
        };
    ...
    fn begin_span(&mut self) -> SpanTracker {
        SpanTracker { code_fragment_id: self.code_fragment_id, start: self.index }
    }
}
...
where
    C: Iterator<Item = char>,
{
    type Item = Result<TokenWithSpan, ParseError>;

    fn next(&mut self) -> Option<Result<TokenWithSpan, ParseError>> {
        if self.is_done {
            None
        } else {
...
pub(crate) fn lex<C>(chars: C, code_fragment_id: CodeFragmentId) -> Lex<C::IntoIter>
where
    C: IntoIterator<Item = char>,
{
    ...
    Lex { chars, ch_0, ch_1, code_fragment_id, index: 0, is_done: false }
}
...
impl SpanTracker {
    fn token<C>(&self, lex: &Lex<C>, token: Token) -> TokenWithSpan {
        TokenWithSpan { span: Span { code_fragment_id: self.code_fragment_id, start: self.start, end: lex.index }, token }
    }

    fn error<C>(&self, lex: &Lex<C>, message: String) -> ParseError {
        ParseError { span: Span { code_fragment_id: self.code_fragment_id, start: self.start, end: lex.index }, message }
    }
}
