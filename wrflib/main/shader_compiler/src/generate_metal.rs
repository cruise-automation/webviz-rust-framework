...
pub fn generate_shader(shader: &ShaderAst) -> String {
    ...
    ShaderGenerator { shader, string: &mut string, backend_writer: &MetalBackendWriter() }.generate_shader();
    string
}
...
impl<'a> ShaderGenerator<'a> {
    fn generate_shader(&mut self) {
        ...
        writeln!(
            self.string,
            "float4 sample2d(texture2d<float> tex, float2 pos){{return \
             tex.sample(sampler(mag_filter::linear,min_filter::linear),pos);}}"
        )
        .unwrap();
        ...
        for &(ty_lit, ref param_tys) in
            vertex_decl.cons_fn_deps.borrow_mut().as_ref().unwrap().union(fragment_decl.cons_fn_deps.borrow().as_ref().unwrap())
        {
                ...
                Decl::Struct(decl) => {
                    ...
                    if !decl.fields.is_empty() {
                        ...
                        for field in &decl.fields {
                            ...
                            self.write_var_decl(false, false, field.ident, field.ty_expr.ty.borrow().as_ref().unwrap());
                            writeln!(self.string, ";").unwrap();
                        }
                    }
                    writeln!(self.string, "}};").unwrap();
    ...
    fn generate_uniform_structs(&mut self) {
        ...
        for decl in &self.shader.decls {
            match decl {
                Decl::Uniform(decl) => {
                    let uniform_block =
                        uniform_blocks.entry(decl.block_ident.unwrap_or(Ident::new("default"))).or_insert(Vec::new());
        ...
        for (ident, decls) in uniform_blocks {
            ...
            for decl in decls {
                write!(self.string, "    ").unwrap();
                self.write_var_decl(false, true, decl.ident, decl.ty_expr.ty.borrow().as_ref().unwrap());
                writeln!(self.string, ";").unwrap();
            }
            writeln!(self.string, "}};").unwrap();
        }
        ...
        for decl in &self.shader.decls {
            match decl {
                Decl::Geometry(decl) if decl.is_used_in_fragment_shader.get().unwrap() => {
                    write!(self.string, "    ").unwrap();
                    self.write_var_decl(false, false, decl.ident, decl.ty_expr.ty.borrow().as_ref().unwrap());
                    writeln!(self.string, ";").unwrap();
                }
    ...
    fn generate_fn_decl(&mut self, decl: &FnDecl, visited: &mut HashSet<IdentPath>) {
        FnDeclGenerator { shader: self.shader, decl, visited, backend_writer: self.backend_writer, string: self.string }
            .generate_fn_decl()
    }
    ...
    fn generate_vertex_main(&mut self) {
        ...
        write!(self.string, ", const device mpsc_Geometries *in_geometries [[buffer(0)]]").unwrap();
        write!(self.string, ", const device mpsc_Instances *in_instances [[buffer(1)]]").unwrap();
        write!(self.string, ", constant mpsc_pass_Uniforms &mpsc_pass_uniforms [[buffer(2)]]").unwrap();
        // TODO(JP): Add back if we want to properly support View-level uniforms.
        // write!(self.string, ", constant mpsc_view_Uniforms &mpsc_view_uniforms [[buffer(3)]]").unwrap();
        write!(self.string, ", constant mpsc_draw_Uniforms &mpsc_draw_uniforms [[buffer(4)]]").unwrap();
        write!(self.string, ", constant mpsc_default_Uniforms &mpsc_default_uniforms [[buffer(5)]]").unwrap();
    ...
    fn generate_fragment_main(&mut self) {
        ...
        write!(self.string, ", constant mpsc_pass_Uniforms &mpsc_pass_uniforms [[buffer(0)]]").unwrap();
        // TODO(JP): Add back if we want to properly support View-level uniforms.
        // write!(self.string, ", constant mpsc_view_Uniforms &mpsc_view_uniforms [[buffer(1)]]").unwrap();
        write!(self.string, ", constant mpsc_draw_Uniforms &mpsc_draw_uniforms [[buffer(2)]]").unwrap();
        write!(self.string, ", constant mpsc_default_Uniforms &mpsc_default_uniforms [[buffer(3)]]").unwrap();
        write!(self.string, ", mpsc_Textures mpsc_textures").unwrap();
...
impl<'a> FnDeclGenerator<'a> {
    fn generate_fn_decl(&mut self) {
        ...
        for param in &self.decl.params {
            write!(self.string, "{}", sep).unwrap();
            self.write_var_decl(param.is_inout, false, param.ident, param.ty_expr.ty.borrow().as_ref().unwrap());
            sep = ", ";
        }
        for &ident in self.decl.uniform_block_deps.borrow().as_ref().unwrap() {
            write!(self.string, "{}constant mpsc_{1}_Uniforms &mpsc_{1}_uniforms", sep, ident).unwrap();
            sep = ", ";
        }
        ...
        if is_used_in_fragment_shader && (has_geometry_deps || has_instance_deps || has_varying_deps) {
            write!(self.string, "{}thread mpsc_Varyings &mpsc_varyings", sep).unwrap();
        }
        write!(self.string, ") ").unwrap();
...
impl BackendWriter for MetalBackendWriter {
    fn write_call_expr_hidden_args(&self, string: &mut String, ident_path: IdentPath, shader: &ShaderAst, sep: &str) {
        let mut sep = sep;
    ...
    fn generate_var_expr(
        ...
        if is_used_in_fragment_shader {
            match kind.get().unwrap() {
                VarKind::Geometry | VarKind::Instance | VarKind::Varying => write!(string, "mpsc_varyings.").unwrap(),
                _ => {}
            }
        }
        match kind.get().unwrap() {
