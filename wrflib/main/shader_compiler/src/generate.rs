...
pub(crate) struct BlockGenerator<'a> {
    pub(crate) shader: &'a ShaderAst,
    pub(crate) decl: &'a FnDecl,
    pub(crate) backend_writer: &'a dyn BackendWriter,
    pub(crate) indent_level: usize,
    pub(crate) string: &'a mut String,
}
...
impl<'a> BlockGenerator<'a> {
    ...
    fn generate_stmt(&mut self, stmt: &Stmt) {
        ...
        match *stmt {
            ...
            Stmt::Let { span, ref ty, ident, ref ty_expr, ref expr } => self.generate_let_stmt(span, ty, ident, ty_expr, expr),
    ...
    fn generate_for_stmt(
        ...
        let from = from_expr.const_val.borrow().as_ref().unwrap().as_ref().unwrap().to_int().unwrap();
        let to = to_expr.const_val.borrow().as_ref().unwrap().as_ref().unwrap().to_int().unwrap();
        let step = if let Some(step_expr) = step_expr {
            step_expr.const_val.borrow().as_ref().unwrap().as_ref().unwrap().to_int().unwrap()
        } else if from < to {
            1
        } else {
            -1
    ...
    fn generate_if_stmt(&mut self, _span: Span, expr: &Expr, block_if_true: &Block, block_if_false: &Option<Box<Block>>) {
...
pub(crate) struct ExprGenerator<'a> {
    pub(crate) shader: &'a ShaderAst,
    pub(crate) decl: Option<&'a FnDecl>,
    pub(crate) backend_writer: &'a dyn BackendWriter,
    pub(crate) string: &'a mut String,
}
    ...
    pub(crate) fn generate_expr(&mut self, expr: &Expr) {
        match (expr.const_val.borrow().as_ref(), expr.const_index.get()) {
            ...
            _ => match expr.kind {
                ...
                ExprKind::MethodCall { span, ident, ref arg_exprs } => self.generate_method_call_expr(span, ident, arg_exprs),
                ExprKind::Field { span, ref expr, field_ident } => self.generate_field_expr(span, expr, field_ident),
                ExprKind::Index { span, ref expr, ref index_expr } => self.generate_index_expr(span, expr, index_expr),
                ExprKind::Call { span, ident_path, ref arg_exprs } => self.generate_call_expr(span, ident_path, arg_exprs),
                ExprKind::ConsCall { span, ty_lit, ref arg_exprs } => self.generate_cons_call_expr(span, ty_lit, arg_exprs),
                ExprKind::Var { span, ref kind, ident_path } => self.generate_var_expr(span, kind, ident_path, &expr.ty.borrow()),
                ExprKind::Lit { span, lit } => self.generate_lit_expr(span, lit),
            },
        }
    }

    fn generate_cond_expr(&mut self, _span: Span, expr: &Expr, expr_if_true: &Expr, expr_if_false: &Expr) {
        write!(self.string, "(").unwrap();
        self.generate_expr(expr);
        write!(self.string, " ? ").unwrap();
    ...
    fn generate_method_call_expr(&mut self, span: Span, ident: Ident, arg_exprs: &[Expr]) {
        match arg_exprs[0].ty.borrow().as_ref().unwrap() {
            Ty::Struct { ident: struct_ident } => {
                self.generate_call_expr(span, IdentPath::from_two(*struct_ident, ident), arg_exprs);
            }
            _ => panic!(),
        }
    }
